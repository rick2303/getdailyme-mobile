...
import { ArrowRight, AtSign, ChevronDown, Lock } from 'lucide-react-native'
...
import { readLastSignInMethod, rememberSignInMethod, usesEmail, type SignInMethod } from '@/lib/auth/last-method'
...
// El enlace magico se queda en la web: en nativo el correo no puede volver a
// la app hasta que el dominio este vivo.
export default function SignInScreen() {
  ...
  const [otherWaysChoice, setOtherWaysChoice] = useState<boolean | null>(null)
  const otherWaysOpen = otherWaysChoice ?? usesEmail(lastMethod)
  const chevronTurn = useSharedValue(0)

  useEffect(() => {
    let cancelled = false
    void readLastSignInMethod().then((method) => {
      if (!cancelled) setLastMethod(method)
    })
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    const target = otherWaysOpen ? 180 : 0
    chevronTurn.value = reducedMotion ? target : withSpring(target, { stiffness: 500, damping: 30 })
  }, [otherWaysOpen, reducedMotion, chevronTurn])
    ...
    try {
      if (creatingAccount) {
        ...
        if (signUpError) {
          ...
          void rememberSignInMethod('password')
        }
      } else {
        ...
        else void rememberSignInMethod('password')
      }
    } catch {
    ...
    try {
      ...
      if (params.code) {
        ...
        if (exchangeError) {
          ...
          else void rememberSignInMethod('google')
        } else {
          void rememberSignInMethod('google')
        }
      } else {
  ...
  const signInWithApple = async () => {
    ...
    try {
      ...
      void rememberSignInMethod('apple')
    ...
    <View className="flex-1 bg-bg dark:bg-bg-dark">
      ...
      <SafeAreaView className="flex-1">
        ...
        >
          ...
          <Animated.View entering={reducedMotion ? undefined : FadeInUp.delay(120).duration(500)} className="gap-3">
          ...
          <Pressable
            ...
            onPress={() => {
              haptic('tap')
              if (otherWaysOpen) setCreatingAccount(false)
              setOtherWaysChoice(!otherWaysOpen)
              setError(null)
            }}
            className="min-h-11 flex-row items-center justify-center gap-1.5 self-center px-3 active:opacity-70"
          >
          ...
          {otherWaysOpen ? (
            <Animated.View
              entering={
                Platform.OS === 'ios' && !reducedMotion && otherWaysChoice !== null
            ...
            >
              ...
              {creatingAccount ? (
                <Pressable
                  ...
                  className="flex-row items-start gap-2.5 px-1 active:opacity-70"
                >
                  <View
                    className={
                      acceptedTerms
